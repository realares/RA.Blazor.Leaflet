...
using RA.Blazor.Leaflet.Models;
using System;
...
using Rectangle = RA.Blazor.Leaflet.Models.Rectangle;

namespace RA.Blazor.Leaflet
{
    public static class LeafletInterops
    {
        ...
        public static ValueTask AddLayer(IJSRuntime jsRuntime, string mapId, Layer layer)
        {
            return layer switch
            {
                WmsTileLayer wmsTileLayer => jsRuntime.InvokeVoidAsync($"{_BaseObjectContainer}.addWmsLayer", mapId, wmsTileLayer, CreateLayerReference(mapId, wmsTileLayer)),
        ...
        => layer switch
        {
            GridLayer gridLayer => js.InvokeVoidAsync($"{_BaseObjectContainer}.setOpacity", mapId, gridLayer.Id, gridLayer.Opacity),
            _ => throw new NotImplementedException($"The layer {typeof(Layer).Name} has not been implemented."),
        };
    }
}
